...
{
    public static class Alterportal2012TopBuilder
    {
        ...
        private static UserTopBlock GetTopByTotalTopNode(HtmlNode node)
        {
            ...
            if (tableNode != null && tableNode.ChildNodes.Count == 11)
            {
                ...
                top.CommentID = top.CommentID.Substring(0, top.CommentID.Length - 10);
                top.CommentUser = userNode.ChildNodes[1].InnerText;
                top.CommentTime = userNode.ChildNodes[2].InnerText;
                HtmlNode regNode =
                    tableNode.ChildNodes[9].ChildNodes[3].ChildNodes[0].ChildNodes[1].ChildNodes[3].ChildNodes[0];
                string regTime = regNode.InnerText;
                int timeStartInd = regTime.IndexOf(":");
                int teimEndInd = regTime.IndexOf("|");
